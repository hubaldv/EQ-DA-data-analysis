...
def plot_different_rpms(filename, title, mean_subtracted=False, ylim=None):
    ...
    for ax, rpm in zip(axes, rpm_vec):
        data = load_data(f'{filename}_{rpm}RPM', length_s=0.2)
        if mean_subtracted:
    ...
    if mean_subtracted:
        plt.savefig(f'figures/png/{filename}_mean_subtracted.png', dpi=300)
        plt.savefig(f'figures/svg/{filename}_mean_subtracted.svg')
    else:
        plt.savefig(f'figures/png/{filename}.png', dpi=300)
        plt.savefig(f'figures/svg/{filename}.svg')
    plt.show()
    

plot_different_rpms('1_parallel_open', title='Parallel Sensors Without Equilibar: Raw Pressure Signals at Different RPMs', ylim=[0, 3.0])
plot_different_rpms('1_parallel_open', title='Parallel Sensors Without Equilibar: Mean Removed Pressure Signals at Different RPMs', mean_subtracted=True, ylim=[-0.25, 0.25])
...
def get_amplitudes(filename, rpm=1000):
    ...
    return dominant_amp_up, dominant_amp_down, peak_amp_4x_up, peak_amp_4x_down
...
result_df = pd.DataFrame(result, columns=['Backpressure [mbar]', 'Dominant Amp Upstream', 'Dominant Amp Downstream',
                                          '4x Freq Amp Upstream', '4x Freq Amp Downstream'])
...
plt.savefig(f'figures/amplitude_vs_backpressure_{rpm}RPM.svg')
plt.show()